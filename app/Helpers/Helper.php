...
class Helper
{
    ...
    {
        $this->data = $data;
        $this->phone_number_format = self::formatPhoneNumer($this->data);
        $this->products_in_a_page = $this->data;
    }
    ...
    protected static function convertPhoneString($strNum)
    {
        $len = strlen($strNum);
        $counter = 3;
        $result = "";
    ...
    protected static function formatPhoneNumer($phone)
    {
        if (strcmp($phone, '') == 0) {
            return '';
        }
        ...
        $area_code = '';
        $area_code = '(+' . str_before($phone, '-') . ') ';

        $phone_number = '';
        $phone_number = self::convertPhoneString(str_after($phone, '-'));

        $full = '';
        $full = $area_code . $phone_number;
